...
The white dwarf temperature approach holds two major advantages over using the donor properties. Measurements of white dwarf temperatures are easier to gather for large sample sizes, and the `zero $\dot M$' $T_{\rm eff}$ is more well-known and more commonly accepted than the `zero $\dot M$' $R_{\rm donor}$ required by the donor properties method.
