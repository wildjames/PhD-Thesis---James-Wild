...
Figure~\ref{fig:introduction:rappaport garraffo matt magnetic braking} \todo{Take out the Garraffo prescription - we can't reproduce the tracks she shows in her paper!} shows the strength of $\dot J$ across a CV's mass range, though below the period gap is not thought to be present. 
The \citet{garraffo2018a} prescription is shown in two forms - as the complexity is assumed to be $n < 7$, there is a region of CV evolution where the assumption in Equation~\ref{eqn:introduction:garraffo complexity modulation} that neglects the second term does not apply. The dotted line in the figure shows the torque on the star is we use $n = 7$ for this parameter space, and the solid line shows the torque if we contunie using the calculated $n$ regardless. 
