...
All eclipses presented are well-described by their fits, with small residuals. The lightcurves are contained in Appendix~\ref{appendix:lightcurves}, along with the white dwarf flux distributions compared to the cooling tracks. The example system of ASASSN-14kb has the lightcurve and flux distribution is shown in Figure~\ref{fig:results:12 new CVs:ASASSN-14kb all lightcurves}, and Figure~\ref{fig:results:12 new CVs:ASASSN-14kb flux plot}, respectively.
...
Some results, however, are good demonstrations of the ability of the hierarchical GP modelling approach to usefully model poorer quality light curves in tandem with higher quality data.
...
Fitting found a somewhat low mass white dwarf, $0.67 \pm 0.07 M_\odot$, falling just over $1\sigma$ outside the \citet{pala2020} range of $0.83\pm0.13 M_\odot$, and a donor mass approximately consistent with the pre-period minimum CV tracks.
...
CSS090419 significantly brightens in the $i'$ band, compared to the $r'$ band. This is unlikely to be calibration error in the $i'$ band, as calibration has otherwise proven to be robust, and this brightening is not seen in the ASASSN-15pb $i'$ band observations. Modelling error can also be eliminated, as the eclipse features are reasonably well resolved.  The white dwarf and bright spot ingresses are somewhat blended in the $r'$ and $i'$, but the egresses are distinct enough to compensate for this. However, the blended features are reflected in large uncertainty in white dwarf flux.
This is not typical white dwarf behaviour, and cannot be reproduced by the white dwarf model atmospheres used here. The disagreement between the observed $i'$ flux and the best-fit model $i'$ flux is relatively small, less than $2\sigma$, so the resulting fit can still be considered valid. The posterior $\pi$ distribution is consistent with the Gaia measurement of $1.41\pm0.78$ mas, with slightly lower uncertainty.
...
Other CV models suggest that the effect of correcting $M_{\rm wd}$ will be small; at most around 2 minutes \citep{goliasch2015}, and this analysis is rudimentary so the slightly incorrect white dwarf mass is considered acceptable.
